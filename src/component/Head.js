  ...
  return (
    <header className="head">
      <nav className='nav sub'>
        {props.path === '/main' ? <div className='headTitle'><img src={logo} alt='MND'/>군 소요물자 신청</div> : user === 'admin' ? <div className='headTitle'><img src={logo} alt='MND'/>군 소요물자 신청관리</div> : <button className='navButton back' onClick={back}><i className="ri-arrow-left-s-line"></i></button>}
        ...
        {isMobile ? <button className='navButton menu' onClick={test}><i className="ri-menu-line"></i></button> : <span className={'spaceButton'} style={{flex: user !== 'admin' && 'none'}}/>}
      </nav>
    </header>
  );
}
